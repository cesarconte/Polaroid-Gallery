...
const toggleLike = async (card) => {
  ...
  const updatedLikedBy = isUserLiked ? likedBy.filter((id) => id !== userId) : [...likedBy, userId]
...
const toggleFollowUser = async (card) => {
  ...
  console.log(`User ${userId} ${isUserFollowing ? 'unfollowed' : 'followed'} user ${cardUserId}`)
