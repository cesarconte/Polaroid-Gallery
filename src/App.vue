...
<template>
  <v-app>
    <v-app-bar flat color="grey-darken-4" scroll-behavior="fade-image"
      scroll-threshold="560" image="https://picsum.photos/1920/1080?random"
      ...
      <template v-slot:prepend>
        <RouterLink active-class="activeIcon" to="/"
          class="text-decoration-none"
          :class="mdAndDown ? 'rounded-circle' : 'rounded-pill'">
          <v-app-bar-nav-icon icon="mdi-panorama-variant-outline" border
            class="router">
          </v-app-bar-nav-icon>
      ...
      <template v-slot:append>
        <RouterLink
          :active-class="mdAndDown ? 'text-black bg-white' : 'text-black bg-white'"
          to="/explore"
          class="router text-decoration-none mr-2 d-none d-md-block"
          :class="mdAndDown ? 'rounded-circle' : 'rounded-pill'">
          <Button v-if="mdAndDown" border btnSize="48" class="rounded-circle">
            <v-icon>mdi-database-search-outline</v-icon>
            <v-tooltip activator="parent" location="bottom">Explore</v-tooltip>
          </Button>
          <Button v-else border :btnText="lgAndUp ? 'Explore' : ''"
        ...
        <RouterLink
          :active-class="mdAndDown ? 'text-black bg-white' : 'text-black bg-white'"
          to="/users" class="router text-decoration-none mr-2 d-none d-md-block"
          :class="mdAndDown ? 'rounded-circle' : 'rounded-pill'">
          <Button v-if="mdAndDown" border btnSize="48"
            class="btnIcon rounded-circle">
            <v-icon>mdi-account-group-outline</v-icon>
            <v-tooltip activator="parent" location="bottom">Users</v-tooltip>
          </Button>
          <Button v-else :btnText="lgAndUp ? 'Users' : ''" border
        ...
        <RouterLink v-if="!isLoggedIn"
          ...
          <Button v-else rounded="xl" border
        ...
        <RouterLink v-if="!isLoggedIn"
          :active-class="mdAndDown ? 'text-black bg-white' : 'active text-black bg-white'"
        ...
        <v-app-bar-nav-icon v-if="isLoggedIn"
          :variant="smAndDown ? '' : 'outlined'" color="white"
          :size="smAndDown ? '48' : ''" class="text-none d-none d-sm-block"
          ...
          <div :class="smAndDown ? '' : 'mr-2'">
            <Avatar :avatarSize="smAndDown ? '44' : '34'">
              <v-img v-if="userData.avatar" cover :src="userData.avatar"
                :alt="isLoggedIn ? `Avatar of ${userData.fullName}` : 'User Avatar'"
                color="grey-darken-3">
              </v-img>
            </Avatar>
          </div>
        ...
        <Button v-if="isLoggedIn && mdAndDown" border
          class="text-none d-none d-md-block mr-2 rounded-circle" btn-size="48"
          @click="handleSignOut">
          <v-icon>mdi-account-arrow-right-outline</v-icon>
          <v-tooltip activator="parent" location="bottom">Log out</v-tooltip>
        </Button>
        <Button v-if="isLoggedIn && lgAndUp" border
          class="mr-2 text-none d-none d-md-block rounded-pill"
          @click="handleSignOut">
          <v-icon
            :class="lgAndUp ? 'mr-2' : ''">mdi-account-arrow-right-outline</v-icon>
          <span v-if="lgAndUp">Log out</span>
        </Button>
        <PublishCard />
        <v-app-bar-nav-icon v-if="smAndUp" border :class="mdAndUp ? 'mr-2' : ''"
          @click="toggleTheme">
          <v-icon>mdi-theme-light-dark</v-icon>
          <v-tooltip activator="parent" location="bottom">Switch to Light/Dark
            mode</v-tooltip>
        </v-app-bar-nav-icon>
      ...
      <v-menu>
        <template v-slot:activator="{ props }" class="px-0">
          <Button class="d-sm-none mr-2 rounded-pill" btnSize="48"
            v-bind="props">
            <v-icon>mdi-dots-vertical</v-icon>
            <v-tooltip activator="parent" location="bottom">Drop Down
              Menu</v-tooltip>
          </Button>
        </template>
        <v-list>
    ...
    <v-navigation-drawer v-model="drawer" app :location="'top'"
      :temporary="true" :class="'h-auto'">
      <v-list>
        <v-list-item
          v-for="(item, index) in navItems.filter(item => !item.showWhenLoggedIn)"
          :key="index" :prepend-icon="item.icon" @click="handleNavItem(item)">
          {{ item.label }}
        </v-list-item>
