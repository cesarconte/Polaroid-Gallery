...
<template>
  <v-responsive>
    <v-container fluid>
      <h2 class="pb-6 mb-4" :class="smAndDown ? 'text-left' : 'text-center'">
        Polaroid Gallery Users Information & Activity
      </h2>
      <v-autocomplete v-model.trim="selectedUser" label="Search User" :items="users" item-title="fullName"
        item-text="fullName" chips closable-chips prepend-inner-icon="mdi-account-outline" variant="outlined" rounded
        auto-select-first="true" :class="sm ? 'w-75 mx-auto' : mdAndUp ? 'w-50 mx-auto' : 'w-full'">
        <template v-slot:chip="{ props, item }">
          <v-chip v-bind="props" :text="item.raw.fullName"></v-chip>
        </template>

        <template v-slot:item="{ props, item }">
          <v-list-item v-bind="props" :title="item.raw.fullName"></v-list-item>
        </template>
      </v-autocomplete>
