...
<script setup>
  ...
  const fetchRandomCards = async () => {
    ...
    const randomCardsQuery = query(
      randomCardsCollection,
      orderBy('randomOrder'),
      limit(6)
    );
    const randomCardsSnapshot = await getDocs(randomCardsQuery);
    randomCards.value = randomCardsSnapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    }));
  };
      ...
      return cards.value.filter((card) => {
        const tagsMatch =
          ...
          selectedTags.value.some(
            (tag) => card.tag?.toLowerCase() === tag.toLowerCase()
          );
        const usersMatch =
          !selectedUsers.value.length ||
          selectedUsers.value.some(
            (user) => card.userFullName?.toLowerCase() === user.toLowerCase()
          );
        const titleMatch =
          !searchTitle.value ||
  ...
  <v-responsive>
    <v-container fluid>
      ...
      <p
        ...
        class="mb-10"
      >
        We hope you find what are you looking for, {{ userData.fullName }}.
      </p>
      <p
        v-else
        class="mb-10"
      >
        We hope you find what are you looking for.
      </p>
      <div
        class="d-block mx-auto"
        :class="xs ? 'w-100' : sm ? 'w-75' : md ? 'w-75' : 'w-50'"
      >
        <v-autocomplete
