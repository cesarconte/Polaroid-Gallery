...
<script setup>
  ...
  const filteredCards = computed(() => {
    ...
    return cards.value.filter((card) =>
      currentUser.value.createdCards.includes(card.id)
    );
  });
  ...
  const userTotalFavorites = computed(() => {
    ...
    return cards.value.filter(
      (card) =>
        currentUser.value.createdCards.includes(card.id) && card.isFavorite
    ).length;
  });
    ...
    <v-container fluid>
                ...
                <v-row>
                  ...
                  <v-col :cols="smAndDown ? '12' : 'auto'">
                    ...
                    <div>
                      ...
                      <p class="ml-8 mb-4">
                        Total My Likes: {{ currentUser.likeCards.length }}
                      </p>
                      <p class="pt-1">
      ...
      >
        <v-row>
          <v-col :cols="smAndDown ? '12' : '9 mx-auto'">
            <v-sheet
              v-if="currentUser && filteredCards.length > 0"
              min-height="31.624vh"
