...
<script setup>
  ...
  import {
    isValidEmail,
    isValidPassword,
  } from '@/services/Auth/authValidationsService';
...
<template>
  <v-container fluid>
    ...
    >
      ...
      />
      <CardSubtitle
        subtitleText="Contact Information"
        class="pl-0 mb-4"
      />
      <FormEmailInput divTxt="Email" />
      <v-alert
        v-if="!isValidEmail(userData.email)"
        dense
        outlined
        type="warning"
        variant="tonal"
      ...
      />
      <v-alert
        v-if="!isValidPassword(userData.password)"
        dense
        outlined
        type="warning"
        variant="tonal"
      ...
      <v-scale-transition>
        <div v-if="!loading">
          <Button
            ...
            variant="tonal"
            btnText="Sign up"
            appendIcon="mdi-account-box-edit-outline"
            @click="handleSignUp"
          />
          <Button
            block
